...
func TestExternalTemplateDataSourceProcessRuntime(t *testing.T) {
	binary := os.Getenv("WEKNORA_TEMPLATE_DATASOURCE_PLUGIN_BIN")
	if binary == "" {
		t.Skip("set WEKNORA_TEMPLATE_DATASOURCE_PLUGIN_BIN to run the template acceptance test")
	}
	if _, err := os.Stat(binary); err != nil {
		t.Fatalf("template plugin binary: %v", err)
	}

	pluginRoot := t.TempDir()
	manifest := Manifest{
		APIVersion:      APIVersionV1,
		ID:              "your-org.example-datasource",
		...
		Permissions:     Permissions{Network: NetworkNone},
		Config:          []ConfigField{{Key: "endpoint", Type: "string", Required: true}},
	...
	config := &types.DataSourceConfig{Type: manifest.ID, Settings: map[string]interface{}{"endpoint": "example"}}
	lease, err := registry.GetForScope(ctx, manifest.ID, datasource.ConnectorScope{DataSourceID: "template-datasource-1"}, config)
	require.NoError(t, err)
	connector := lease.Connector()
	all, err := connector.FetchAll(ctx, config, nil)
	require.NoError(t, err)
	require.Len(t, all, 1)
	require.Equal(t, "template:document", all[0].ExternalID)

	changed, cursor, err := connector.FetchIncremental(ctx, config, nil)
	require.NoError(t, err)
	require.Len(t, changed, 1)
	require.NotNil(t, cursor)
	changed, _, err = connector.FetchIncremental(ctx, config, cursor)
	require.NoError(t, err)
