...
WeKnora 里已经有两块很完整的拼图：

| 拼图 | 它知道什么 | 已有的能力 |
|---|---|---|
| **用户长期记忆** | 这个人反复用过哪些文档 | 从对话里提取记忆、按人检索、在重排阶段做个性化加权 |
| **Wiki 知识网络** | 这片知识由哪些节点组成、怎么互链 | 把文档蒸馏成互链页面与知识图谱 |

**但它们各走各的。** 结果是一个很奇怪的状态：

> 系统其实**同时知道**「这片知识长什么样」和「这个人用过哪些文档」，
> 却从没把这两件事对上过——**用户自己看不到「我在这片知识里，走到哪了」。**

举个具体的场景。一个用户读完了一批关于「注意力机制」的文档，系统内部是有记录的（引用过、打开过、停留过）。但当他把 Wiki 图谱打开时，看到的仍然是一张**按页面类型上色的静态图**：

- 他不知道哪些节点是自己**真正熟悉**的，哪些只是**扫过一眼**；
- 他不知道自己**还剩哪些盲区**，也不知道**下一步最该看什么**；
- 读完的那些内容**没有任何反馈**，关掉页面就等于没发生过。

### 1.2 由此产生的三个具体困境

| # | 困境 | 现状 | 后果 |
|---|---|---|---|
| 1 | **看不见自己的位置** | 图谱按页面类型上色，与「我」无关 | 用户无法区分「我熟悉的」和「我没碰过的」 |
| 2 | **看不见盲区** | 没有任何「这里你还没碰过」的标记 | 不知道下一步该往哪走，探索靠随机点 |
| 3 | **看不见积累** | 读完即消失，没有沉淀 | 缺乏持续回访的动力，学习没有正反馈 |

这三个问题的共同根源是：**知识网络是静态的，而人的学习是动态的**。图谱画的是「这片知识是什么」，但它没有回答「**我**和这片知识是什么关系」。

### 1.3 我们的答案：把行为投影到知识网络上

我们加了一层**个人知识地图**——把用户自己的行为（**读过什么、引用过什么、认可过什么**）投影到 Wiki 知识网络上：
...
| 能力 | 回答的问题 | 用户看到的东西 |
|---|---|---|
| **地图** | 我在这片知识空间里的**位置** | 每个节点是一个球，球内水位 = 我的证据强度 |
| **画像** | 我的水位**是由什么构成的** | 明细表：这个节点被引用几次、浏览多久、点赞几次 |
| **引导** | 我的**下一步**该看哪里 | 点击节点时，相邻的边界节点亮起涟漪 + 「建议先看」列表 |

### 1.4 一条必须先说清楚的边界

> 这套水位表达的是**「用户与这个知识节点的关联强度与近期活跃度」**，
> **不是对用户真实认知能力的判断。**
...
- 「掌握」只是一个 UI 标签，它的语义是「**高证据活跃**」。

我们选择把这条边界**写在最前面**，而不是藏在附录里——因为它决定了后面每一个算法决策的合理性：我们做的是一个**行为证据的累积器**，不是一个**认知能力的评分器**。
...
| **点赞** | 间接认可：用户认可整条回答 | 这条回答是否有用 | `LikeWeight = 2` | 次线性封顶，按位置分摊 |
