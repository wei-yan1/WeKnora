...
WeKnora 已经有两块很完整的拼图：一块是**用户长期记忆**，它知道这个人反复用过哪些文档；另一块是 **Wiki 知识网络**，它知道这片知识由哪些节点组成、彼此怎么互链。但这两块拼图**各走各的**——系统同时知道「这片知识长什么样」和「这个人用过哪些文档」，却从没把这两件事对上过。结果是：用户打开 Wiki 图谱时，看到的仍然是一张按页面类型上色的**静态图**，与「我」无关。

于是用户看不见自己的位置（哪些是真正熟悉的、哪些只是扫过一眼），看不见盲区（还剩哪些没碰过），也看不见积累（读完就消失，没有任何反馈）。这三个问题的共同根源是：**知识网络是静态的，而人的学习是动态的**——图谱画的是「这片知识是什么」，却没有回答「**我**和这片知识是什么关系」。

我们加的这一层叫**个人知识地图**：把用户自己的行为（读过什么、引用过什么、认可过什么）投影到 Wiki 知识网络上，让每个节点亮起属于自己的水位。它同时回答三件事——**地图**回答「我在哪里」，**画像**回答「我的水位是由什么构成的」，**引导**回答「下一步该看哪里」。

最后有一条必须先说清楚的边界：这套水位表达的是**「用户与这个知识节点的关联强度与近期活跃度」，而不是对用户真实认知能力的判断**。引用不等于理解，浏览不等于读懂，点赞认可的是「回答」而非「逐篇文档」，被检索到不等于用户看到。所以产品侧统一叫「个人知识状态」或「熟悉证据水位」，「掌握」只是一个 UI 标签，语义是「**高证据活跃**」。我们把它写在最前面，是因为它决定了后面每一个算法决策的合理性——我们做的是一个**行为证据的累积器**，不是一个**认知能力的评分器**。
