...
>
> 阅读前置：先读 [plugin-development-datasource.md](plugin-development-datasource.md) 了解统一的插件骨架（`plugin.yaml` 字段、`PluginControl` 握手/健康、`WEKNORA_PLUGIN_ADDR`、目录环境变量、构建与装载）。本文只讲 Parser 特有的部分，不重复通用骨架。
